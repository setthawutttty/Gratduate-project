...
const Market = () => {
    ...
    const fetchApi = async (datauser) => {
        ...
        await axios.get(`${url}/api/market?id=${datauser.user.market_id}`).then((res) => {
            setData(res.data.data[0])
            console.log(res.data.data[0])
            setDatatable([])
        })
        
        console.log(`${url}/api/market/listimage?market_id=${datauser.user.market_id}`)
        await axios.get(`${url}/api/market/listimage?market_id=${datauser.user.market_id}`).then((res) => {
  ...
  const submitForm = async () => {
    ...
    await axios.put(`${url}/api/market?id=${data.id}`, {
        ...
        if (data.data.code === 200) {  
          ...
          console.log(`${url}/api/users?id=${userdata2.id}`) 
          axios.put(`${url}/api/users?id=${userdata2.id}`, {
            name : userdata2.name,
            email : userdata2.email,
            permission : userdata2.permission,
            tel : userdata2.tel
        }).then(data => {
          console.log(data.data)
            if (data.data.code === 200) {   
            ...
            <Row>
              ...
              <Col sm='6' className='mb-1'>
                ...
                <Input id='emailInput' type='email' name='email' placeholder='อีเมล'
                    onChange={(e) => setUserdata2({...userdata2, usersEmail : e.target.value}) }
                    value={userdata2.email} disabled
                    />
              </Col>
              <Col sm='6' className='mb-1'>
                ...
                <Input id='company' name='company' placeholder='ชื่อผู้ดูแล' 
                    onChange={(e) => setUserdata2({...userdata2, name : e.target.value}) }
              ...
              <Col sm='6' className='mb-1'>
                ...
                <Input id='company' name='company' placeholder='เบอร์โทรศัพท์' 
                    onChange={(e) => setUserdata2({...userdata2, tel : e.target.value}) }
                    value={userdata2.tel}                        
                    />
              </Col>
